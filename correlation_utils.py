...
def compute_correlation_result(merged, *, factor: str, party: str, year: int, mode: str) -> dict:
    ...
    if n < 10:
        logger.warning(
            ...
            factor, party, year, None, n, mode, "insufficient_rows"
        )
    ...
    if cleaned["share"].nunique(dropna=True) < 2:
        logger.warning(
            "Invalid correlation result | factor=%s party=%s year=%s r=%r n=%s mode=%s reason=%s",
            factor, party, year, None, n, mode, "zero_variance_share"
        )
    ...
    if cleaned["metric"].nunique(dropna=True) < 2:
        logger.warning(
            "Invalid correlation result | factor=%s party=%s year=%s r=%r n=%s mode=%s reason=%s",
            factor, party, year, None, n, mode, "zero_variance_metric"
        )
    ...
    if not is_valid_correlation(raw_r):
        logger.warning(
            "Invalid correlation result | factor=%s party=%s year=%s r=%r n=%s mode=%s reason=%s",
            factor, party, year, raw_r, n, mode, "non_finite_or_out_of_range"
        )
